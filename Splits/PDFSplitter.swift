...
class PDFSplitter {
  ...
  init(pdf: CGPDFDocument) throws {
    ...
    pageImage_ = Observable.combineLatest(pdf_, numberOfPages__.asObservable(),
                                          pageNumber__.asObservable()) { ($0, $1, $2) }
      .filter { _, numberOfPages, pageNumber in
        1 ... numberOfPages ~= pageNumber
      }
      .map { (pdf, _, pageNumber) -> CGPDFPage? in
        pdf.page(at: pageNumber)
      }
      .map { page in guard
        let page = page else { return nil }
  ...
  private static func makeImageForPage(_ page: CGPDFPage) -> CGImage? {
    ...
    guard let context = CGContext(data: nil, width: Int(destBox.size.width), height: Int(destBox.size.height), bitsPerComponent: 8,
  ...
  func split(destUrl: URL) -> Observable<Int> {
    return Observable.combineLatest(pdf_.take(1), numberOfPages_.take(1)) { pdf, numberOfPages -> (CGPDFDocument, Int, CGContext) in
      ...
      var docBox = CGRect(origin: mediaBox.origin, size: CGSize(width: mediaBox.size.width / 2, height
    ...
    .flatMap {
      pdf, numberOfPages, context in Observable.range(start: 1, count: numberOfPages)
